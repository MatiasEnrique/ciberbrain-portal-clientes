...
export default function PerfilForm({
  ...
  const form = useForm<ProfileSchemaType>({
    ...
    defaultValues: {
      ...
      country: user.Pais || 1,
      province: user.Provincia || 0,
      partido: user.IDPartido || 0,
      city: user.Localidad || "",
      postalCode: user.CodigoPostal || "",
      street: user.Calle || "",
      streetNumber: user.Altura || "",
      otherAddressDetails: user.Otros || "",
      crossStreet1: user.Entre || "",
      crossStreet2: user.Entre2 || "",
      preferredTime: user.Horario || "",
      birthDate: user.Nacimiento
        ...
        : "",
      phone: user.Sms || "",
      mobilePhone: user.Sms || "",
    },
  });

  const [isPending, startTransition] = useTransition();

  const { data: provincias } = useQuery<ProvinciaSchemaType[]>({
  ...
  const { data: partidos } = useQuery<PartidoSchemaType[]>({
